...
public class ConsultaVentasFXController implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle rb) {
    ...
    for (int i = 0; i < ves.listaVentas.size(); i++) {
      v = ves.listaVentas.get(i);
      infoArea.appendText(v.toStringVoid());   
      for (int j = 0; j < v.listaProductos.size(); j++) {
  ...
  @FXML
  private void ventaPorFecha(ActionEvent event) {
    if (!fechaTF.getText().isEmpty()){
    for (int i = 0; i < ves.listaVentas.size(); i++) {
      v = ves.listaVentas.get(i);
      if (v.getFecha().equals(fechaTF.getText())){
      infoArea.appendText(v.toStringVoid());   
      for (int j = 0; j < v.listaProductos.size(); j++) {
        infoArea.appendText(v.listaProd(j));
      }
